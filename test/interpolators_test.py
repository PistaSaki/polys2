...
def test_evaluate_interpolator_in_graph_mode():
    ...
    x_sample = [[1, 2], [2, 3], [3, 4]]
    assert np.allclose(interpolate_and_evaluate(x_sample),
                       tf_interpolate_and_evaluate(x_sample))
...
def test_interpolator__gradient():
    params = [tf.linspace(0., np.pi, 10)]
    itp = Interpolator.from_fun(params, fun=tf.sin)
